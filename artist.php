...
<main id="main">
  ...
  <section class="relative overflow-hidden" style="padding-top:calc(var(--header-h) + 4rem)">
    <div class="shell pb-16">
      <nav class="label-xs text-muted flex items-center gap-2 mb-10" aria-label="Breadcrumb">
        <a href="index.php" class="ul-draw">Home</a><span class="text-ink">/</span>
        <a href="artists.php" class="ul-draw">Artists</a><span class="text-ink">/</span>
        <span class="text-muted"><?= e($artist['name']) ?></span>
      </nav>

      <div class="grid lg:grid-cols-12 gap-10 items-end">
        <div class="lg:col-span-8">
          <p class="label-xs text-ink mb-6" data-reveal="right"><?= e($artist['discipline']) ?></p>
          <h1 class="display d-hero" data-split="lines" data-reveal="fade"><?= e($artist['name']) ?></h1>
          <p class="ital d-2 mt-8 max-w-[34ch] text-ink" data-reveal="up" data-reveal-delay="260">“<?= e($artist['statement']) ?>”</p>
        </div>
        ...
        <div class="lg:col-span-4 flex lg:justify-end" data-reveal="scale" data-reveal-delay="200">
          <div class="relative w-40 h-40 sm:w-52 sm:h-52 overflow-hidden shrink-0"
            ...
            <span class="absolute inset-0 grid place-items-center display leading-none text-[4.5rem] sm:text-[6rem] text-white/90"><?= e($artist['initials']) ?></span>
      ...
      <dl class="grid grid-cols-2 sm:grid-cols-4 gap-6 mt-14 pt-8 rule-t" data-reveal-group="80">
        <div data-reveal="up">
          <dt class="label-xs text-muted mb-2">Based in</dt>
          <dd class="display d-4"><?= e($artist['city']) ?></dd>
        </div>
        <div data-reveal="up">
          <dt class="label-xs text-muted mb-2">Born</dt>
          <dd class="display d-4 num"><?= (int) $artist['born'] ?></dd>
        </div>
        <div data-reveal="up">
          <dt class="label-xs text-muted mb-2">With us since</dt>
          <dd class="display d-4 num"><?= (int) $artist['represented'] ?></dd>
        </div>
        <div data-reveal="up">
          <dt class="label-xs text-muted mb-2">Works held</dt>
          <dd class="display d-4 num"><?= count($works) ?> <span class="text-sm text-muted">(<?= count($available) ?> available)</span></dd>
        </div>
      </dl>
    </div>
  </section>
  ...
  <section class="shell py-20 sm:py-28">
    <div class="grid lg:grid-cols-12 gap-10 lg:gap-16">
      <div class="lg:col-span-7">
        <p class="label-xs text-muted mb-7" data-reveal="right">Practice</p>
        ...
        <figure class="mt-12 pt-8 rule-t" data-reveal="up">
          ...
          <figcaption class="label-xs text-ink mt-4">— <?= e($artist['press'][1]) ?></figcaption>
        </figure>
      </div>

      <div class="lg:col-span-5">
        <p class="label-xs text-muted mb-7" data-reveal="right">Selected exhibitions</p>
        <ol class="grid" data-reveal-group="80">
          <?php foreach ($artist['exhibitions'] as $show): ?>
            <li class="flex gap-6 py-5 rule-t" data-reveal="up">
              <span class="label-xs text-ink num shrink-0 pt-1"><?= (int) $show[0] ?></span>
              <span>
                <span class="display d-4 block"><?= e($show[1]) ?></span>
                <span class="label-xs text-muted mt-1.5 block"><?= e($show[2]) ?></span>
              </span>
            </li>
          <?php endforeach; ?>
          <li class="rule-t"></li>
        ...
        <div class="mt-9" data-reveal="up">
          <a href="<?= e(wa_link('Hello Nuqta, I would like to know more about ' . $artist['name'] . '\'s work.')) ?>"
             target="_blank" rel="noopener" class="btn btn-ghost w-full"><span>Ask about this artist</span></a>
        </div>
      </div>
    </div>
  </section>
  ...
  <!-- ======================================================== next up -->
  <section class="shell pb-24 sm:pb-32">
    <p class="label-xs text-muted mb-8" data-reveal="right">Other artists</p>
    <div class="flex flex-wrap gap-x-3 gap-y-4" data-reveal-group="60">
      <?php foreach ($others as $other): ?>
        <a href="artist.php?slug=<?= e($other['slug']) ?>"
           class="group flex items-center gap-3 pr-5 py-2 rule-t flex-1 min-w-[240px]" data-reveal="up">
          <span class="w-10 h-10 shrink-0 grid place-items-center display text-sm text-white"
                style="background:var(--char)"><?= e($other['initials']) ?></span>
          <span class="min-w-0">
            <span class="block display d-4 truncate group-hover:text-muted transition-colors"><?= e($other['name']) ?></span>
            <span class="block label-xs text-muted mt-0.5 truncate"><?= e($other['discipline']) ?></span>
          </span>
        </a>
      <?php endforeach; ?>
    </div>
