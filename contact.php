...
<main id="main">
  ...
  <section class="shell pb-14" style="padding-top:calc(var(--header-h) + 5rem)">
    <p class="label-xs text-muted flex items-center gap-3 mb-6" data-reveal="right">
      <span class="num">04</span><span class="w-8 h-px bg-line"></span>Contact
    </p>
  ...
  <section class="shell pb-20">
    <div class="grid sm:grid-cols-3 gap-px rule-t rule-b" data-reveal-group="90">
      <a href="<?= e(wa_link()) ?>" target="_blank" rel="noopener" class="group p-7 sm:p-8 sm:border-r sm:border-r-[var(--hair)]" data-reveal="up">
        <p class="label-xs text-muted mb-4">Fastest</p>
        <p class="display d-3 group-hover:text-muted transition-colors">WhatsApp</p>
        <p class="label-xs text-muted mt-3 num"><?= e(PHONE_DISPLAY) ?></p>
        <p class="prose-note text-sm mt-4">Usually answered within the hour, Tuesday to Saturday.</p>
      </a>

      <a href="mailto:<?= e(EMAIL) ?>" class="group p-7 sm:p-8 sm:border-r sm:border-r-[var(--hair)]" data-reveal="up">
        <p class="label-xs text-muted mb-4">For anything on paper</p>
        <p class="display d-3 group-hover:text-muted transition-colors">Email</p>
        <p class="label-xs text-muted mt-3"><?= e(EMAIL) ?></p>
        <p class="prose-note text-sm mt-4">Condition reports, invoices, valuations and submissions.</p>
      </a>

      <a href="<?= e(map_link()) ?>" target="_blank" rel="noopener" class="group p-7 sm:p-8" data-reveal="up">
        <p class="label-xs text-muted mb-4">Best of all</p>
        <p class="display d-3 group-hover:text-muted transition-colors">Come in</p>
        <p class="label-xs text-muted mt-3"><?= e(ADDRESS_LINE_1) ?>, <?= e(ADDRESS_LINE_2) ?></p>
  ...
  <section class="shell pb-24 sm:pb-32">
    <div class="grid lg:grid-cols-12 gap-10 lg:gap-16 items-start">
      ...
      <div class="lg:col-span-7">
        <form data-simple-form novalidate class="rule-t rule-b bg-paper p-6 sm:p-9">
          ...
          <div class="flex flex-wrap items-center gap-4 mt-9 pt-7 rule-t">
            ...
            <p class="label-xs text-muted">No newsletter sign-up hidden in here.</p>
          </div>
        </form>

        <div data-form-done hidden class="rule-t rule-b bg-paper p-7 sm:p-10">
          ...
          <div class="grid sm:grid-cols-2 gap-3 mt-8">
            <a class="btn" href="<?= e(wa_link()) ?>" target="_blank" rel="noopener" data-wa-form><span>Send on WhatsApp</span></a>
            <a class="btn btn-ghost" href="mailto:<?= e(EMAIL) ?>"><span>Send by email</span></a>
          </div>
        </div>
      </div>
      ...
      <aside class="lg:col-span-5 grid gap-10">
        ...
        <div class="rule-t pt-7" data-reveal="up">
          <p class="label-xs text-muted mb-5">Follow the work</p>
          <ul class="grid gap-3">
            <?php foreach (SOCIALS as $social): ?>
              <li class="flex items-baseline justify-between gap-4 pb-2.5 rule-b">
                ...
                <span class="label-xs text-muted"><?= e($social['handle']) ?></span>
              </li>
            <?php endforeach; ?>
          </ul>
        </div>
        ...
        <div class="rule-t pt-7" data-reveal="up">
          <p class="label-xs text-muted mb-5">Press</p>
          ...
          <a href="mailto:<?= e(EMAIL) ?>?subject=Press%20request" class="btn btn-ghost btn-sm mt-5"><span>Request press images</span></a>
        </div>
      </aside>
    </div>
  </section>
        ...
        <?php foreach (FAQ as $i => $item): ?>
          <div class="acc-item" data-reveal="up" data-reveal-delay="<?= $i * 60 ?>">
            <button type="button" class="acc-btn" aria-expanded="false">
              ...
              <span class="acc-sign text-ink" aria-hidden="true"></span>
            </button>
