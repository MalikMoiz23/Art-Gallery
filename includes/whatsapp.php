<div class="wa-fab">
  ...
  <button type="button" class="wa-trigger" aria-expanded="false" aria-label="Open WhatsApp options">
  ...
  <div class="wa-card">
    <div class="flex items-start gap-3">
      ...
      <div>
        ...
        <p class="text-xs text-muted mt-0.5">Replies Tue–Sat, 11:00–19:00 PKT</p>
      </div>
    </div>
    ...
    <div class="grid gap-1.5 mt-4">
      ...
      foreach ($prompts as $label => $message): ?>
        <a href="<?= e(wa_link($message)) ?>" target="_blank" rel="noopener"
           class="flex items-center justify-between gap-3 px-3 py-2.5 rule-t text-xs text-muted hover:text-ink hover:bg-mist transition-colors">
          <span><?= e($label) ?></span>
    ...
    <p class="label-xs text-muted mt-4 num"><?= e(PHONE_DISPLAY) ?></p>
  </div>
</div>
