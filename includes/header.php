<header class="site-header">
  <div class="shell flex items-center justify-between gap-6" style="height:var(--header-h)">
    ...
    <a href="index.php" class="flex items-baseline gap-2 shrink-0" data-cursor="tight" aria-label="<?= e(SITE_FULL) ?>, home">
      <span class="display text-[1.55rem] leading-none tracking-[-0.03em]"><?= e(SITE_NAME) ?></span>
      <span class="w-[7px] h-[7px] rounded-full bg-brass-500 mb-[3px]"></span>
      <span class="hidden xl:inline label-xs text-paper-500 ml-2 pl-3 rule-l">Est. <?= e((string) SITE_EST) ?> · Islamabad</span>
    </a>

    <nav class="hidden lg:flex items-center gap-9" aria-label="Primary">
      <?php foreach (NAV as $item): ?>
        <a href="<?= e($item['file']) ?>"
           class="group flex items-baseline gap-2 text-sm <?= is_current($item['file']) ? 'text-brass-500' : 'text-paper-200' ?>"
           <?= is_current($item['file']) ? 'aria-current="page"' : '' ?>>
          <span class="label-xs text-paper-500 group-hover:text-brass-500 transition-colors"><?= e($item['index']) ?></span>
    ...
    <div class="flex items-center gap-2 sm:gap-3">
      <a href="<?= e(wa_link()) ?>" target="_blank" rel="noopener" class="icon-btn hidden sm:grid" aria-label="Message us on WhatsApp" data-cursor="tight" data-no-veil>
      ...
      <button type="button" class="icon-btn relative" data-drawer-open aria-label="Open enquiry list" data-cursor="tight">
...
<div class="menu-sheet" id="menu-sheet">
  <div class="shell flex flex-col justify-between py-8 h-full" style="padding-top:calc(var(--header-h) + 2rem)">
    ...
    <div class="m-item grid sm:grid-cols-3 gap-6 pt-8">
      <div>
        <p class="label-xs text-paper-500 mb-2">Visit</p>
        <p class="text-sm"><?= e(ADDRESS_LINE_1) ?><br><?= e(ADDRESS_LINE_2) ?></p>
      </div>
      <div>
        <p class="label-xs text-paper-500 mb-2">Talk</p>
        <p class="text-sm num"><?= e(PHONE_DISPLAY) ?><br><?= e(EMAIL) ?></p>
      </div>
      <div class="flex sm:justify-end items-start">
        <a href="<?= e(wa_link()) ?>" target="_blank" rel="noopener" class="btn btn-ghost btn-sm" data-no-veil><span>WhatsApp us</span></a>
      </div>
    </div>
  </div>
</div>
