...
<main id="main">
  ...
  <nav class="shell pb-8 label-xs text-paper-500 flex items-center gap-2 flex-wrap" style="padding-top:calc(var(--header-h) + 2.5rem)" aria-label="Breadcrumb">
    <a href="index.php" class="ul-draw">Home</a><span class="text-brass-500">/</span>
    <a href="gallery.php" class="ul-draw">Collection</a><span class="text-brass-500">/</span>
    <a href="gallery.php?category=<?= urlencode($work['category']) ?>" class="ul-draw"><?= e($work['category']) ?></a><span class="text-brass-500">/</span>
    <span class="text-paper-300"><?= e($work['title']) ?></span>
  </nav>
    ...
    <div class="grid lg:grid-cols-12 gap-10 lg:gap-16 items-start">
      ...
      <div class="lg:col-span-7 lg:sticky" style="top:calc(var(--header-h) + 1.5rem)">
        <button type="button" class="block w-full text-left group relative spot"
                data-zoom="<?= e(img_src($work['img'])) ?>" data-cursor="Zoom" aria-label="Enlarge <?= e($work['title']) ?>">
          <div class="frame">
          ...
          <span class="absolute bottom-4 right-4 z-10 label-xs px-3 py-1.5 bg-ink-950/70 backdrop-blur text-paper-300 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            Click to enlarge
          </span>
        </button>

        <div class="flex flex-wrap items-center justify-between gap-4 mt-4 label-xs text-paper-500">
      ...
      <div class="lg:col-span-5">
        <div class="flex items-center gap-3 mb-6" data-reveal="right">
          ...
          <span class="label-xs text-paper-500"><?= e($work['category']) ?> · <?= (int) $work['year'] ?></span>
        </div>
        ...
        <?php if ($artist): ?>
          <a href="artist.php?slug=<?= e($artist['slug']) ?>" class="inline-flex items-center gap-3 mt-5 group" data-reveal="up" data-reveal-delay="140">
            <span class="w-9 h-9 grid place-items-center display text-sm shrink-0"
                  style="background:linear-gradient(150deg, <?= e($artist['accent']) ?>, #12100e)"><?= e($artist['initials']) ?></span>
            <span>
              <span class="block text-sm group-hover:text-brass-500 transition-colors"><?= e($artist['name']) ?></span>
              <span class="block label-xs text-paper-500 mt-0.5"><?= e($artist['discipline']) ?> · <?= e($artist['city']) ?></span>
            </span>
          </a>
        <?php endif; ?>
        ...
        <div class="mt-9 pt-7 rule-t" data-reveal="up" data-reveal-delay="200">
          <div class="flex items-end justify-between gap-4">
            <div>
              <p class="label-xs text-paper-500 mb-2">Price</p>
              <p class="display d-2 num <?= $sold ? 'text-paper-500 line-through' : 'text-brass-500' ?>"><?= e(money($work['price'])) ?></p>
            </div>
            <p class="label-xs text-paper-500 text-right leading-relaxed max-w-[22ch]">
          ...
          <div class="grid gap-2.5 mt-8">
            <?php if (!$sold): ?>
              ...
              <div class="flex gap-2.5">
                <a href="<?= e(wa_link($waMessage)) ?>" target="_blank" rel="noopener" class="btn btn-ghost flex-1" data-no-veil>
            ...
            <?php else: ?>
              ...
              <a href="<?= e(wa_link('Hello Nuqta, "' . $work['title'] . '" is sold — what else by ' . ($artist['name'] ?? '') . ' is available?')) ?>"
                 target="_blank" rel="noopener" class="btn btn-ghost w-full" data-no-veil><span>Ask what else is available</span></a>
            <?php endif; ?>
          </div>

          <p class="label-xs text-paper-500 mt-5 leading-relaxed">
        ...
        <dl class="mt-10" data-reveal-group="55">
          <?php foreach ($specs as $key => $value): ?>
            <div class="flex items-baseline justify-between gap-6 py-3.5 rule-t" data-reveal="up">
              <dt class="label-xs text-paper-500 shrink-0"><?= e($key) ?></dt>
        ...
        <div class="mt-10" data-reveal="up">
          <p class="label-xs text-brass-600 mb-4">On this work</p>
          <p class="prose-note"><?= e($work['story']) ?></p>
        </div>

        <ul class="flex flex-wrap gap-2 mt-7" data-reveal="up">
          <?php foreach ($work['tags'] as $tag): ?>
            <li><a href="gallery.php" class="chip hover:border-brass-500/50 hover:text-brass-500 transition-colors"><?= e($tag) ?></a></li>
          <?php endforeach; ?>
        </ul>
      </div>
    </div>
    ...
    <section class="plaster text-ink-900 py-20 sm:py-28">
      <div class="shell grid lg:grid-cols-12 gap-10 items-center">
        <div class="lg:col-span-7">
          <p class="label-xs text-clay-600 mb-6" data-reveal="right">The artist, in their words</p>
          <blockquote class="display d-1 text-ink-900 max-w-[26ch]" data-split="lines" data-reveal="fade">“<?= e($artist['statement']) ?>”</blockquote>
          <p class="text-ink-700 mt-8 max-w-[56ch] leading-relaxed" data-reveal="up" data-reveal-delay="140"><?= e($artist['bio'][0]) ?></p>
          <a href="artist.php?slug=<?= e($artist['slug']) ?>" class="btn btn-solid-dark mt-9" data-reveal="up" data-magnet="7"><span>More on <?= e($artist['name']) ?></span></a>
        </div>

        <?php if ($siblings): ?>
          <div class="lg:col-span-5">
            <p class="label-xs text-ink-600 mb-5">Also available by <?= e($artist['name']) ?></p>
            <div class="grid grid-cols-2 gap-4" data-reveal-group="80">
              <?php foreach (array_slice($siblings, 0, 2) as $sib): $sm = media($sib['img']); ?>
                <a href="artwork.php?slug=<?= e($sib['slug']) ?>" class="group" data-reveal="up" data-cursor="View work">
                  <span class="media grade block" style="aspect-ratio:<?= (int) $sm['w'] ?>/<?= (int) $sm['h'] ?>">
                    <?= picture($sib['img'], $sib['title']) ?>
                  </span>
                  <span class="block display d-4 text-ink-900 mt-3 group-hover:text-clay-600 transition-colors"><?= e($sib['title']) ?></span>
                  <span class="block label-xs text-ink-600 mt-1 num"><?= e(money_short($sib['price'])) ?></span>
                </a>
              <?php endforeach; ?>
            </div>
          </div>
  ...
  <section class="shell pb-24">
    <div class="grid sm:grid-cols-2 gap-px rule-t rule-b">
      <a href="artwork.php?slug=<?= e($prev['slug']) ?>" class="group flex items-center gap-5 py-8 sm:pr-8" data-cursor="tight">
        <span class="text-brass-500 group-hover:-translate-x-1 transition-transform duration-500" aria-hidden="true">←</span>
        <span>
          <span class="label-xs text-paper-500 block mb-1.5">Previous</span>
          <span class="display d-3 group-hover:text-brass-500 transition-colors"><?= e($prev['title']) ?></span>
        </span>
      </a>
      <a href="artwork.php?slug=<?= e($next['slug']) ?>" class="group flex items-center justify-end gap-5 py-8 sm:pl-8 sm:border-l sm:border-l-[var(--hair)] text-right" data-cursor="tight">
        <span>
          <span class="label-xs text-paper-500 block mb-1.5">Next</span>
          <span class="display d-3 group-hover:text-brass-500 transition-colors"><?= e($next['title']) ?></span>
        </span>
        <span class="text-brass-500 group-hover:translate-x-1 transition-transform duration-500" aria-hidden="true">→</span>
      </a>
    </div>
